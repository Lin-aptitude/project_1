		...
		this.each(function(){
			...
			var zoom = {
				_init:function(){
					...
					this.$smallImg[0].onload=()=>{
						// 大图位置
						if(opt.position === 'right'){
							this.$big.css({
								left:$small.offset().left+this.$smallImg.outerWidth()+opt.gap,
								top:$small.offset().top
							})
							console.log(this.$big.offset().left);
				...
				show:function(){
					// 写入图片地址
					this.$bigImg.attr('src',this.$smallImg.attr('dataBig'));
